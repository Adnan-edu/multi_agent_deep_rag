...
async def cleanup():
    """Clear space and remove installed dependencies before exiting."""
    print("\nCleaning up...")
    try:
...
async def finance_research(query, instructions=None):
    try:
        ...
        result = await agent.ainvoke({"messages": [HumanMessage(full_query)]})
